...
in
    if ActiveSeasonCount <> 1 then
        error
            if ActiveSeasonCount = 0 then
                "No Active season found in Static_Season_Config (SeasonStatus=Active)."
            else
                "Multiple Active seasons found in Static_Season_Config. Expected exactly 1."
    else
        Typed
...
Version History
    ...
    - Added ActiveSeasonCount <>1 test clause after in to warn the user if there
      are zero or more than one active season in Static_Season_Config.
